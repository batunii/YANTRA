    ...
    private var header: some View {
        HStack {
            Button { path.removeLast() } label: {
                Image(systemName: "chevron.left").font(.system(size: 17, weight: .semibold)).foregroundStyle(y.secondary)
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                        ...
                        if case let .device(d) = item, d.repeating {
                            Image(systemName: "repeat").font(.system(size: 9)).foregroundStyle(y.dim)
                        }
