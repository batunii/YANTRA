...
interface NodeDao {
    ...
    /** How many rows the index actually holds — see [ie.shoonya.yantra.data.workspace.Indexer]. */
     ...
     * [ie.shoonya.yantra.AppContainer]. The index is the only place a forgotten workspace could
     ...
     * [ie.shoonya.yantra.domain.FocusTimer] — which is a different and much stronger claim than
...
interface FocusDao {
     ...
     * both, and owes them under one name — see [ie.shoonya.yantra.data.db.counts].
     */
...
interface LabelDao {
     ...
     */
    @Query("SELECT COUNT(DISTINCT node_id) FROM node_label WHERE label_id IN (:labelIds)")
    suspend fun countUsage(labelIds: List<String>): Int
