...
import ie.shoonya.yantra.widget.actions.FocusAction
 ...
 *
 * A launcher stores the `ComponentName` of the provider each placed widget belongs to, so renaming
 * this class breaks every widget already on a home screen — it does not move, it stops resolving,
 * and it has to be placed again. That is why it was called Pomodoro long after the feature stopped
 * being called that: the class name is invisible to the user and the widget disappearing is not.
 *
 * The application id changed in the same commit as this rename, which breaks every one of those
 * bindings regardless. The cost was already being paid, so the name is now the one the rest of the
 * app uses.
 */
