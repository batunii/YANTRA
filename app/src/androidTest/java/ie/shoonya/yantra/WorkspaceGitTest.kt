...
class WorkspaceGitTest {
    ...
    private fun Git.commitAll(msg: String) {
        ...
        commit().setMessage(msg)
            .setAuthor("Yantra", "yantra@shoonya.ie")
            .setCommitter("Yantra", "yantra@shoonya.ie")
            .call()
    }
    ...
    fun aLockLeftByAKilledProcessDoesNotBreakSyncForever() {
        ...
        val repo = ie.shoonya.yantra.data.sync.GitRepo(dir, "yantra-tasks")
        repo.init().use { git ->
            buildWorkspace(dir)
            repo.commitAll(git, "first", "Yantra", "yantra@shoonya.ie")
        }
