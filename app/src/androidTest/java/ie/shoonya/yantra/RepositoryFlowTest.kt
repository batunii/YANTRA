...
class RepositoryFlowTest {
    ...
    private lateinit var focus: ie.shoonya.yantra.data.repo.FocusRepository
    ...
    fun setUp(): Unit = runBlocking {
        ...
        focus = ie.shoonya.yantra.data.repo.FocusRepository(db, ws)
    }
    ...
    fun aBlockCreatedThenIndentedThenOutdented() = runBlocking {
        ...
        assertTrue(pageText(task)!!.contains("${ie.shoonya.yantra.data.format.PageCodec.INDENT} second"))
    ...
    fun aTypedLineBecomesATaskWithEverythingItSaid() = runBlocking {
        ...
        assertEquals("High", db.propertyDao().valuesForNodeOnce(id).firstOrNull {
            it.defId == ie.shoonya.yantra.data.db.BuiltIns.PRIORITY_DEF_ID
        }?.vText)
        assertTrue(
            "no due date was set",
            db.propertyDao().valuesForNodeOnce(id).any {
                it.defId == ie.shoonya.yantra.data.db.BuiltIns.DUE_DEF_ID && it.vDate != null
            },
        )
    ...
    fun plainTypingIsLeftExactlyAsWritten() = runBlocking {
        ...
        assertTrue(db.propertyDao().valuesForNodeOnce(id).none {
            it.defId == ie.shoonya.yantra.data.db.BuiltIns.DUE_DEF_ID
        })
    }
